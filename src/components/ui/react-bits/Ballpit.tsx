...
class W {
  ...
  constructor(config: WConfig) {
    ...
    this.flightTimer = new Float32Array(config.count).fill(0);
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      if (this.flightTimer[idx] > 0) {
        this.flightTimer[idx] -= 1;
        vel.clampLength(0, config.flightBoostVelocity ?? config.maxVelocity);
      } else {
        if (config.flightChance && Math.random() < config.flightChance) {
          this.flightTimer[idx] = config.flightDuration ?? 40;
          const burst = new Vector3(
            ...
            .normalize()
            .multiplyScalar(config.flightForce ?? 0.6);
          vel.add(burst);
        }
